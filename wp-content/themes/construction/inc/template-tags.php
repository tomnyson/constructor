 ...
 */
function wpcharming_get_header() {

	global $wpc_option;
	$header_name = null;

	if ( isset( $wpc_option['header_style'] ) ) {
		$header_layout = $wpc_option['header_style'];
	} else {
		$header_layout = '';
	}
	if ( $wpc_option['header_style'] == 'topbar' || $wpc_option['header_style'] == 'centered') {
		$header_name = $wpc_option['header_style'];
	} else {
		$header_name = '';
	}

	if ( isset( $_REQUEST['header-demo']) && ($_REQUEST['header-demo'] == 'topbar' ) ) $header_name = 'topbar';
		...
		<div class="page-header-wrap clearfix '. $wpc_row_parallax .'"'. $page_header_style . $data_bg .'>
			...
			<div class="container">';
				...
				if ( $header_title !== '' ) {
					...
					//echo '<h1 class="page-title"'. $heading_color .'>'. get_the_title() .'</h1>';
				}
...
if ( ! function_exists('wpcharming_get_page_title') ) :
/**
 * Display the page title area at the top of single page with option.
 */
function wpcharming_get_page_title($postID) {
	global $wpc_option;
	global $woocommerce;
	...
	if ( $hide_page_title != 'on' ) {
		...
		<div class="page-title-wrap <?php if ( $display ) echo 'page-title-has-button'; ?>">
			<div class="container">
				<h1 class="page-entry-title <?php if ( $display ) echo 'left'; ?>">
					<?php the_title(); ?>
				</h1>
				<?php
				if ( $display ) echo $button;
				?>
			</div>
		</div>
		<?php
...
function wpcharming_posted_on() {
	...
	echo '<span class="byline"> '. __('Posted by ', 'wpcharming') . $byline . '</span><span class="posted-on">'. __(' on ', 'wpcharming') . $posted_on . '</span>';
 ...
 */
function wpcharming_list_child_pages( $pageID, $order, $orderby, $exclude, $layout, $column, $number, $readmore_text, $carousel_autoplay, $carousel_autoplay_speed, $carousel_speed ) {
	...
	if ( $page_childrens->have_posts() ) :
		...
		//$excerpt = get_the_excerpt() ? get_the_excerpt() : wp_trim_words( apply_filters('the_content', get_the_content() ),20);
