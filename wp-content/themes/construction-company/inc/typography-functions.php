...
function construction_company_fonts_url(){
    $fonts_url = '';
    
    $primary_font         = get_theme_mod( 'construction_company_primary_font', 'PT Sans' );
    $ig_primary_font      = construction_company_is_google_font( $primary_font );     
    
    /* Translators: If there are characters in your language that are not
    * supported by respective fonts, translate this to 'off'. Do not translate
    * into your own language.
    */
    $primary      = _x( 'on', 'Primary Font: on or off', 'construction-company' );
    ...
    if ( 'off' !== $primary ) {
        ...
        if ( 'off' !== $primary && $ig_primary_font ) {
            $primary_variant = construction_company_check_varient( $primary_font, 'regular', true );
            if( $primary_variant ){
 ...
 * Get Google Fonts
*/
function construction_company_get_google_fonts(){
...
*/
function construction_company_get_websafe_font(){
...
*/
function construction_company_check_varient( $font_family = 'serif', $font_variants = 'regular', $body = false ){
    $variant = '';
    $var     = array();
    $google_fonts  = construction_company_get_google_fonts(); //Google Fonts
    $websafe_fonts = construction_company_get_websafe_font(); //Standard Web Safe Fonts
...
*/
function construction_company_get_css_variant( $font_variant ){
    $v_array = array(
        '100'       => array(
            'weight'    => '100',
            'style'     => 'normal'
...
*/
function construction_company_is_google_font( $font ){
    $return = false;
    $websafe_fonts = construction_company_get_websafe_font();
    if( $font ){
...
*/
function construction_company_get_fonts( $font_family, $font_variant = "regular" ){
    
    $fonts = array();
    $websafe_fonts = construction_company_get_websafe_font(); //Web Safe Font
    
    if( $font_family ){
        if( construction_company_is_google_font( $font_family ) ){
            $fonts['font'] = esc_attr( $font_family ); //Google Font
            if( $font_variant ){
                $weight_style    = construction_company_get_css_variant( construction_company_check_varient( $font_family, $font_variant ) );
