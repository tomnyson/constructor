...
/*
add_filter('wpc_theme_remove_vc_elements', 'childtheme_vc_elements');
function childtheme_vc_elements(){
	$remove_elements = '';
...
// Remove VC unwanted elements
function wpc_theme_remove_unwanted_vc_row_params() {
	...
	// Apply filters for tweaking
	$remove_params = apply_filters( 'wpc_theme_remove_vc_row_params', $remove_params );

	// Remove VC elements
	if ( is_array( $remove_params ) ) {
		foreach ( $remove_params as $remove_param ) {
			vc_remove_param("vc_row", $remove_param);
		}
	}
}
add_action( 'admin_init', 'wpc_theme_remove_unwanted_vc_row_params' );
